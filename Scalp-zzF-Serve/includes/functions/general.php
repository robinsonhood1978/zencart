...
//pic1
      ...
      if ($duplicate_image->fields['total'] < 2 and $product_image->fields['products_image'] != '' && PRODUCTS_IMAGE_NO_IMAGE != substr($product_image->fields['products_image'], strrpos($product_image->fields['products_image'], '/')+1)) {
          ...
          $filename_small = 'small/' . $products_image_base . IMAGE_SUFFIX_MEDIUM . $products_image_extension;
      ...
      //pic2
      $product_image2 = $db->Execute("select products_image2
                                   from " . TABLE_PRODUCTS . "
                                   where products_id = " . (int)$product_id);

      $duplicate_image2 = $db->Execute("select count(*) as total
                                     from " . TABLE_PRODUCTS . "
                                     where products_image2 = '" . zen_db_input($product_image2->fields['products_image2']) . "'");

      if ($duplicate_image2->fields['total'] < 2 and $product_image2->fields['products_image2'] != '' && PRODUCTS_IMAGE_NO_IMAGE != substr($product_image2->fields['products_image2'], strrpos($product_image2->fields['products_image2'], '/')+1)) {
          $products_image2 = $product_image2->fields['products_image2'];
          $products_image_extension2 = substr($products_image2, strrpos($products_image2, '.'));
          $products_image_base2 = preg_replace('/' . $products_image_extension2 . '/', '', $products_image2);

          $filename_small2 = 'small/' . $products_image_base2 . IMAGE_SUFFIX_MEDIUM . $products_image_extension2;
          $filename_medium2 = 'medium/' . $products_image_base2 . IMAGE_SUFFIX_MEDIUM . $products_image_extension2;
          $filename_large2 = 'large/' . $products_image_base2 . IMAGE_SUFFIX_LARGE . $products_image_extension2;

          if (file_exists(DIR_FS_CATALOG_IMAGES . $filename_small2)) {
              @unlink(DIR_FS_CATALOG_IMAGES . $filename_small2);
          }
          if (file_exists(DIR_FS_CATALOG_IMAGES . $filename_medium2)) {
              @unlink(DIR_FS_CATALOG_IMAGES . $filename_medium2);
          }
          if (file_exists(DIR_FS_CATALOG_IMAGES . $filename_large2)) {
              @unlink(DIR_FS_CATALOG_IMAGES . $filename_large2);
          }
      }

      //pic3
      $product_image3 = $db->Execute("select products_image3
                                   from " . TABLE_PRODUCTS . "
                                   where products_id = " . (int)$product_id);

      $duplicate_image3 = $db->Execute("select count(*) as total
                                     from " . TABLE_PRODUCTS . "
                                     where products_image3 = '" . zen_db_input($product_image3->fields['products_image3']) . "'");

      if ($duplicate_image3->fields['total'] < 2 and $product_image3->fields['products_image3'] != '' && PRODUCTS_IMAGE_NO_IMAGE != substr($product_image3->fields['products_image3'], strrpos($product_image3->fields['products_image3'], '/')+1)) {
          $products_image3 = $product_image3->fields['products_image3'];
          $products_image_extension3 = substr($products_image3, strrpos($products_image3, '.'));
          $products_image_base3 = preg_replace('/' . $products_image_extension3 . '/', '', $products_image3);

          $filename_small3 = 'small/' . $products_image_base3 . IMAGE_SUFFIX_MEDIUM . $products_image_extension3;
          $filename_medium3 = 'medium/' . $products_image_base3 . IMAGE_SUFFIX_MEDIUM . $products_image_extension3;
          $filename_large3 = 'large/' . $products_image_base3 . IMAGE_SUFFIX_LARGE . $products_image_extension3;

          if (file_exists(DIR_FS_CATALOG_IMAGES . $filename_small3)) {
              @unlink(DIR_FS_CATALOG_IMAGES . $filename_small3);
          }
          if (file_exists(DIR_FS_CATALOG_IMAGES . $filename_medium3)) {
              @unlink(DIR_FS_CATALOG_IMAGES . $filename_medium3);
          }
          if (file_exists(DIR_FS_CATALOG_IMAGES . $filename_large3)) {
              @unlink(DIR_FS_CATALOG_IMAGES . $filename_large3);
          }
      }
