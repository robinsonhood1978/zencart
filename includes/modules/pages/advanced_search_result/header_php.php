...
if (isset($_GET['m_brand']) && $_GET['m_brand'] != '') {
    $m_brand = $_GET['m_brand'];
    if (isset($_GET['series_code']) && $_GET['series_code'] != '') {
        $series_code = $_GET['series_code'];
        if (isset($_GET['model_code']) && $_GET['model_code'] != '') {
            $model_code = $_GET['model_code'];
            $where_str .= " and p.products_id in (select product_id from fortech_brand_series a join fortech_series_model b on 
            a.brand_series_id=b.brand_series_id join fortech_model c on b.series_model_id = c.series_model_id where 
            a.brand_name='".$m_brand."' and b.series_code='".$series_code."' and c.model_code='".$model_code."')";
        }
        else{
            $where_str .= " and p.products_id in (select product_id from fortech_brand_series a join fortech_series_model b on 
            a.brand_series_id=b.brand_series_id where a.brand_name='".$m_brand."' and b.series_code='".$series_code."')";
        }

    }
    else{
        $where_str .= " and p.products_id in (select product_id from fortech_brand_series where brand_name='".$m_brand."')";
    }

}
if (isset($_GET['p_brand']) && $_GET['p_brand'] != '') {
    $p_brand = $_GET['p_brand'];
    if (isset($_GET['part_code']) && $_GET['part_code'] != '') {
        $part_code = $_GET['part_code'];
        $where_str .= " and p.products_id in (select product_id from fortech_brand_part c join fortech_part d on 
            c.brand_part_id=d.brand_part_id where c.brand_name='".$p_brand."' and d.part_code='".$part_code."')";
    }
    else{
        $where_str .= " and p.products_id in (select product_id from fortech_brand_part where brand_name='".$p_brand."')";
    }
}
