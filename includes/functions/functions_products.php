...
//Robin
function zen_get_product_ext_details($product_id, $language_id = null)
{
    global $db;
...
function zen_get_product_extro_msg($msg_group,$msg_id)
{
    global $db;

    $sql = "SELECT p.message_text
            FROM " . TABLE_PRODUCTS_EXTRA_MESSAGE . " p 
            WHERE p.message_group = '" . $msg_group . "' and p.msg_id = " . (int)$msg_id . "
            LIMIT 1";
    $product = $db->Execute($sql);
    return $product->fields['message_text'];
}
