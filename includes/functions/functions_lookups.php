 ...
 */
  function zen_get_products_manufacturers_name($product_id) {
    ...
    //Robin
    function zen_get_products_type_name($product_id) {
        global $db;

        $product_query = "select t.type_name
                          from " . TABLE_PRODUCTS_EXT . " p, " .
            TABLE_TYPE . " t
                          where p.products_id = '" . (int)$product_id . "'
        ...
        return ($product->RecordCount() > 0) ? $product->fields['type_name'] : "";
    }

function zen_get_products_condition_description($product_id) {
    global $db;

    $product_query = "select t.condition_description
                          from " . TABLE_PRODUCTS_EXT . " p, " .
        TABLE_PRODUCTS_CONDITION . " t
                          where p.products_id = '" . (int)$product_id . "'
                          and p.condition_id = t.condition_id";

    $product =$db->Execute($product_query);

    return ($product->RecordCount() > 0) ? $product->fields['condition_description'] : "";
}

function zen_get_products_color($product_id) {
    global $db;

    $product_query = "select t.pcolors_name
                          from " . TABLE_PRODUCTS_EXT . " p, " .
        TABLE_PRODUCTS_COLOR . " t
                          where p.products_id = '" . (int)$product_id . "'
                          and p.color_id = t.pcolors_id";

    $product =$db->Execute($product_query);

    return ($product->RecordCount() > 0) ? $product->fields['pcolors_name'] : "";
}
/*
