...
/* END: ADD MY CUSTOM DETAILS */
  ...
  function send_order_email($zf_insert_id = null, $zf_mode = FALSE) {
    ...
    if ($send_customer_email === true) {
        zen_mail($this->customer['firstname'] . ' ' . $this->customer['lastname'], $this->customer['email_address'], EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . sprintf("KB%06d", $zf_insert_id), $email_order, STORE_NAME, EMAIL_FROM, $html_msg, 'checkout', $this->attachArray);
    }
    ...
    if (SEND_EXTRA_ORDER_EMAILS_TO != '') {
      ...
      zen_mail('', SEND_EXTRA_ORDER_EMAILS_TO, SEND_EXTRA_NEW_ORDERS_EMAILS_TO_SUBJECT . ' ' . EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . sprintf("KB%06d", $zf_insert_id),
